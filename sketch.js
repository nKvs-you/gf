...
// SOUND helper
function playPop() {
  // intentionally silent placeholder to avoid audio node errors after oscillator removal
}
...
function ensureMusicPlaying(forceStart = false) {
  if (!musicEnabled || !bgMusicLoaded || !bgMusic) return;
  if (getAudioContext().state !== 'running') return;

  if (forceStart && !bgMusic.isPlaying()) {
    bgMusic.play();
  }
  if (!bgMusic.isPlaying() && musicEnabled) {
    bgMusic.play();
  }
  bgMusic.setVolume(0.18, 0.1);
}
